...
def train(train_loader, model, criterion, optimizer, epoch, device, length):
    wandb_log_output = defaultdict(list)
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        optimizer.step()
        if batch_idx % 200 == 0:
            cur_time = time.perf_counter() - t1
            batch_time.update(cur_time)
            t1 = time.perf_counter()
            progress.display(batch_idx)
        if rank == 0:
            cur_time = time.perf_counter() - t2
            t2 = time.perf_counter()
            # wandb.log({
            #     "loss/train": loss.item(),
            #     "acc/train": acc,
            #     "sec/batch": cur_time}, commit=False)
            wandb_log_output["loss/train"].append(loss.item())
            wandb_log_output["acc/train"].append(acc)
            wandb_log_output["sec/batch"].append(cur_time)
    return train_loss.avg, train_acc.avg, batch_time.avg, wandb_log_output
...
def validate(val_loader, model, criterion, device, length):
    ...
    for data, target in val_loader:
        ...
        if rank == 0:
            # wandb.log({
            #     "loss/val": loss.item(),
            #     "acc/val": acc}, commit=False)
            wandb_log_output["loss/val"].append(loss.item())
            wandb_log_output["acc/val"].append(acc)

    progress.display(length)
    return val_loss.avg, val_acc.avg, wandb_log_output
...
def main():
    ...
    parser.add_argument('--data_path',
                        type=str,
                        default="./data/webdatasets/mnist/16_datas",
                        help='dataset path')
    ...
    if args.use_wds:
        ...
        train_tars = sorted(glob.glob(os.path.join(args.data_path, "train", "*.tar")))
        ...
        val_tars = sorted(glob.glob(os.path.join(args.data_path, "val", "*.tar")))
    ...
    print_rank("load setup complete")
    ...
    print_rank("model setup complete")

    # data_plot = defaultdict(list)
    for epoch in range(args.epochs):
        model.train()
        train_loss, train_acc, batch_time, train_wandb_log = train(
            train_loader, model, criterion, optimizer, epoch, device, train_length)
        val_loss, val_acc, val_wandb_log = validate(val_loader, model, criterion,
                                                    device, val_length)
        if rank == 0:
            ...
            train_keys = list(train_wandb_log.keys())
            train_len = len(train_wandb_log[train_keys[0]])
            val_keys = list(val_wandb_log.keys())
            val_len = len(train_wandb_log[val_keys[0]])
            ...
            for i in range(max_len):
                ...
                for k in val_keys:
                    if i < val_len:
                        wandb_dict[k] = val_wandb_log[k][i]
                wandb.log(wandb_dict)

            # wandb.log({
            #     'avg/train_loss': train_loss,
            #     'avg/train_acc': train_acc,
            #     'avg/val_loss': val_loss,
            #     'avg/val_acc': val_acc,
            #     'avg/batch_time': batch_time
            # })
    # if rank == 0:
