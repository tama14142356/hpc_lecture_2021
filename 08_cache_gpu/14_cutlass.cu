...
__global__ void kernel(int dim_m, int dim_n, int dim_k,
  ...
  tile_a = &d_a[(a_k * lda + (a_m + offset_a_m)) * ItemsPerVector];
  tile_b = &d_b[((b_n + offset_b_n) * ldb + b_k) * ItemsPerVector];
  ...
  for (int kk = 0; kk < dim_k; kk += Ktile) {
	...
	block_a[a_k + i * ThreadsPerKtile][a_m * ItemsPerVector + j] = tile_a[stride_a + i * ThreadsPerKtile * lda * ItemsPerVector + j];
	block_b[b_k * ItemsPerVector + j][b_n + i * ThreadsPerNtile] = tile_b[stride_b + i * ThreadsPerNtile * ldb * ItemsPerVector + j];
      }
    }
    __syncthreads();
    stride_a += lda * Ktile * ItemsPerVector;
    stride_b += Ktile;
#pragma unroll
