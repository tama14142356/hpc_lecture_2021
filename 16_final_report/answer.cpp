...
void matmult(vector<float> &subA, vector<float> &subB, vector<float> &subC, int N, double &comp_time, double &comm_time, int rank, int size) {
...
int main(int argc, char **argv) {
    ...
    double err = 0;
    for (int i = 0; i < N; i++)
        for (int j = 0; j < N; j++) err += fabs(C[N * i + j]);
    if (rank == 0) {
